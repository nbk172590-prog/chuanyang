...
export function ProductGrid({ activeCategory, activePrice, searchTerm }: ProductGridProps) {
    ...
    try {
      ...
      const data: Product[] = snapshot.docs.map((doc) => {
        ...
        const product: Product = {
          ...
          category: docData.category || 'Tất cả',

          price: typeof docData.price === 'number' ? docData.price : 0,

          originalPrice:
            typeof docData.originalPrice === 'number' ? docData.originalPrice : undefined,
          ...
          discountPrice: docData.discountPrice,
        };
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchCategory = selectedCategory === 'tất cả' || productCategory === selectedCategory;

    // PRICE
    let matchPrice = true;

    if (activePrice === '0 - 3.000.000') {
      matchPrice = product.price >= 0 && product.price <= 3000000;
    }
    ...
    if (activePrice === '10.000.000 - 30.000.000') {
      matchPrice = product.price > 10000000 && product.price <= 30000000;
    }
